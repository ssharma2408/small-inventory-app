    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sliders.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slider_text') ? 'is-invalid' : '' }}" type="text" name="slider_text" id="slider_text" value="{{ old('slider_text', '') }}">
            ...
            <div class="form-group">
                <label class="required" for="slider_img">{{ trans('cruds.slider.fields.slider_img') }}</label>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slider_order') ? 'is-invalid' : '' }}" type="number" name="slider_order" id="slider_order" value="{{ old('slider_order', '0') }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('slider_status') ? 'is-invalid' : '' }}" name="slider_status" id="slider_status" required>
                    ...
                    @foreach(App\Models\Slider::SLIDER_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('slider_status', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
