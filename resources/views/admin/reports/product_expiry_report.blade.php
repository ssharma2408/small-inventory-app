                        ...
                        @foreach ($expense_items as $key => $expense_item)
                            ...
                            <tr style="background-color: {{ $bgcolor }}">
                                ...
                                <td>
                                    ...
                                    @else
                                        Expire in
                                        {{ round((strtotime($expense_item->exp_date) - strtotime(date('Y-m-d'))) / (60 * 60 * 24)) - 1 }}
                                        days
                                    @endif
                                </td>
                                <td>
                                    <a class="btn btn-xs btn-primary"
                                        href="{{ route('admin.inventories.show', $expense_item->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                    <a class="btn btn-xs btn-info"
                                        href="{{ route('admin.inventories.edit', $expense_item->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                </td>
                            </tr>
