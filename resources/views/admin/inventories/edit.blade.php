    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.inventories.update', [$inventory->id]) }}"
            ...
            <div class="row">
                <div class="col-lg-3 col-md-3">
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('supplier') ? 'is-invalid' : '' }}"
                            ...
                            <option value="{{ $id }}" {{ (old('supplier_id') ? old('supplier_id') : $inventory->
                                supplier->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-lg-3 col-md-3">
                    <div class="form-group">
                        <label class="required" for="invoice_number">{{ trans('cruds.inventory.fields.invoice_number')
                            }}</label>
                        <input class="form-control {{ $errors->has('invoice_number') ? 'is-invalid' : '' }}" type="text"
                            name="invoice_number" id="invoice_number"
                            value="{{ old('invoice_number', $inventory->invoice_number) }}" step="1" disabled>
                ...
                <div class="col-lg-3 col-md-3">
                    <div class="form-group">
                        ...
                        <select class="form-control {{ $errors->has('days_payable_outstanding') ? 'is-invalid' : '' }}"
                            ...
                            <option value disabled {{ old('days_payable_outstanding', null)===null ? 'selected' : '' }}>
                                {{ trans('global.pleaseSelect') }}</option>
                            @foreach (App\Models\Inventory::DAYS_PAYABLE_OUTSTANDING_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('days_payable_outstanding', $inventory->
                                days_payable_outstanding) === (string) $key ? 'selected' : '' }}>
                                {{ $label }}</option>
                            @endforeach
                        </select>
                        ...
                        @endif
                        <span class="help-block">{{ trans('cruds.inventory.fields.days_payable_outstanding_helper')
                            }}</span>
                    </div>
                </div>
                <div class="col-lg-3 col-md-3">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('po_file') ? 'is-invalid' : '' }}" type="file"
                            name="po_file" id="po_file" value="{{ old('po_file', $inventory->po_file) }}" />
                            ...
                            if (!empty($expense_items)) {
                                ...
                                foreach ($expense_items as $expense_item) {
                                    ...
                                    echo '<input class="form-check-input cb ml-0" type="checkbox" name="is_box[]" ' .
                                        ...
                                        $expense_item->box_size .
                                        '" name="package_val[]" />
                                                                                                										<input type="hidden" id="box_or_unit" value="' .
                                        $box_unit .
                                        '" name="box_or_unit[]" />
                                                                                                									</div>
                                                                                                									<div class="col-md-1">
                                                                                                										<input class="form-control stock" type="number" name="item_stock[]" value="' .
                                        $expense_item->stock .
                                        '" />
                                                                                                									</div>
                                                                                                									<div class="col-md-1">
                                                                                                										<input class="form-control price" type="text" name="item_price[]" value="' .
                                    ...
                                    echo '</select>
                                        ...
                                        '" />
                                                                                                									...
                                                                                                									<div class="col-md-2">
                                                                                                										<input class="form-control exp_date" type="date" name="item_exp_date[]" value="' .
                                        $expense_item->exp_date .
                                        '" required />
                                                                                                									</div>
                                                                                                									<div class="col-md-1">';
                                    ...
                                    if (!$cnt) {
                                        ...
                                        echo '<button class="btn btn-danger remove_row" id="remove_row">Remove</button>';
                                    }
            ...
            <div class="row">
                <div class="form-group col-lg-3 col-md-3 col-sm-12">
                    <label class="required" for="expense_total">{{ trans('cruds.inventory.fields.expense_total')
                        }}</label>
                    <input class="form-control {{ $errors->has('expense_total') ? 'is-invalid' : '' }}" type="number"
                        name="expense_total" id="expense_total"
                        value="{{ old('expense_total', $inventory->expense_total) }}" step="0.01" required readonly>
                ...
                <div class="form-group col-lg-3 col-md-3 col-sm-12">
                    ...
                    <input class="form-control {{ $errors->has('expense_tax') ? 'is-invalid' : '' }}" type="number"
                        name="expense_tax" id="expense_tax" value="{{ old('expense_tax', $inventory->expense_tax) }}"
                ...
                <div class="form-group col-lg-3 col-md-3 col-sm-12">
                    ...
                    <div class="form-check col-lg-4 ml-3 {{ $errors->has('discount_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="discount_type_{{ $key }}" name="discount_type"
                            value="{{ $key }}" {{ old('discount_type', $inventory->discount_type) === (string) $key ?
                        'checked' : '' }}
                        required>
                ...
                <div class="form-group col-lg-3 col-md-3 col-sm-12">
                    ...
                    <input class="form-control {{ $errors->has('discount') ? 'is-invalid' : '' }}" type="number"
                        name="discount" id="discount" value="{{ old('discount', $inventory->discount) }}" step="0.01">
                ...
                <div class="form-group col-lg-3 col-md-3 col-sm-12">
                    ...
                    <input class="form-control {{ $errors->has('final_price') ? 'is-invalid' : '' }}" type="number"
                        name="final_price" id="final_price" value="{{ old('final_price', $inventory->final_price) }}"
                        step="0.01" required readonly>
