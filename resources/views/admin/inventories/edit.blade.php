            ...
            <form method="POST" action="{{ route('admin.inventories.update', [$inventory->id]) }}"
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('supplier') ? 'is-invalid' : '' }}"
                                ...
                                @foreach ($suppliers as $id => $entry)
                                    <option value="{{ $id }}"
                                        {{ (old('supplier_id') ? old('supplier_id') : $inventory->supplier->id ?? '') == $id ? 'selected' : '' }}>
                                        {{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control {{ $errors->has('days_payable_outstanding') ? 'is-invalid' : '' }}"
                                ...
                                @foreach (App\Models\Inventory::DAYS_PAYABLE_OUTSTANDING_SELECT as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('days_payable_outstanding', $inventory->days_payable_outstanding) === (string) $key ? 'selected' : '' }}>
                                        {{ $label }}</option>
                                @endforeach
                            </select>
                    ...
                    @foreach (App\Models\Inventory::DISCOUNT_TYPE_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('discount_type') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="discount_type_{{ $key }}"
                                ...
                                {{ old('discount_type', $inventory->discount_type) === (string) $key ? 'checked' : '' }}
                                required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('discount') ? 'is-invalid' : '' }}" type="number"
                        name="discount" id="discount" value="{{ old('discount', $inventory->discount) }}"
                        step="0.01">
